...
export class TranslationSet {
  ...
  add(key: Key, locale: Locale, localization: string) {
  ...
  toTypeScript(): string {
    ...
    generatedFileContent += `export type Locale = ${Array.from(this.locales)
      .map((key) => JSON.stringify(key))
      .join(" | ")};\n\n`;
    generatedFileContent += `export type LocalizedKey = ${Array.from(
    ...
    generatedFileContent +=
      "export const LOCALIZATION: {[key in LocalizedKey]: LocalizationItem} = {\n";
